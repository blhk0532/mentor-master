...
test('a user can register', function () {
    ...
    Volt::test('auth.register')
        ->set('name', 'Mentor')
        ->set('email', 'mentor@example.com')
        ->set('password', 'password')
        ->set('passwordConfirmation', 'password')
        ->call('register')
        ->assertRedirect('/');

    expect(User::whereEmail('mentor@example.com')->exists())->toBeTrue();
    expect(Auth::user()->email)->toEqual('mentor@example.com');
...
test('see email hasnt already been taken validation message as user types', function () {
    User::factory()->create(['email' => 'mentor@example.com']);
